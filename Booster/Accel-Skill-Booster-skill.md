...
> 版本: v1.1.0 | 2026-08-16 | 文件: `booster.py`(宿主, 单文件纯标准库) + `skill.md`(本文档)
...
| 检查点 | 规则 |
|---|---|
| 1a变体 | md含单元测试清单且 **verified()**(VERIFIED_RE匹配PASS行 + DENY_RE排除 not/no pass) 通过; 无→UNVERIFIED 不参与组合 |
| 2合并 | merged.md verified() 通过; 无→组合排除, 不入审查 |
...
- **PASS结构化**: VERIFIED_RE(兼容 - [x] 清单项) + DENY_RE, 防 "NOT PASS" 糊弄; 合并验收与1a验收同用 verified()
...
- v1.1 回归 (2026-08-16, --mock): N=2 X=2 → 1a=4变体 → 1b淘汰 → 4组合有差异 → 合并4/4过 → 审查SCORE=8/9/10/8 → combo_2(10) → final.md
...
2. 无限轮次依赖API空tool_calls收敛, 宿主无轮数兜底（真调用注意超时）
3. 某阶段全部失败 → 宿主报错终止
