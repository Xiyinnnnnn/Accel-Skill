...
> 版本: v1.2.0 | 2026-08-16 | 文件: `trial.py`(宿主, 单文件纯标准库) + `skill.md`(本文档)
...
> v1.2.0 变更: 孤儿文件防护(_stray) / 文件清单绝对路径 / 覆盖保护realpath归一化 / --max-turns兜底 / goals-file友好报错
...
- **孤儿文件防护(v1.2)**: 子代理写错文件名/未按计划落盘 → 轮次收尾自动移入该轮 `_stray/` 子目录(证据链保留), 下一轮仅统计计划产出(`agent_<pi>.md`), 杜绝残留文件导致不收敛
...
- **孤儿文件防护(v1.2)**: 每轮收尾把非计划md移入 `_stray/`; 下一轮cur仅=计划产出, 杜绝残留文件导致的伪死循环
- **文件清单绝对路径(v1.2)**: 各阶段消息中md清单为绝对路径, 子代理可直接read_file
- **覆盖保护归一化(v1.2)**: wrote按realpath记录, 同会话异形路径重写自己不再误拒
...
- 退出码: 0成功 / 1失败（资料目录不存在 / 非mock无key / 阶段2零产出 / 某轮零产出）
...
- v1.2 回归 (2026-08-16, --mock): 正常收敛 3→2→1 (total=8); 孤儿实验(子代理写错文件名)同样收敛且孤儿入 `_stray/`; `--max-turns 1` 触发"阶段1无产出"终止
- 安全边界全过: 越权写/非md/覆盖他人/越权读全拒绝
- 真实调用前建议先 `--mock` 验证流程

## 九、边界与坑
1. 重复跑同一work: round1已有md触发覆盖保护 → 用新work或先清空(宿主不提供清理)
2. 无限轮次依赖API返回空tool_calls收敛; 默认无轮数上限(设计), 真调用可用 --max-turns 兜底
